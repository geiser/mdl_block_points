    ...
    function definition()
	{
		...
		foreach($restrictions as $restriction)
		{
			if($restriction->type == 0) // Restrição por pontos
			{
				...
				$html .= '<tr><td>Os pontos do aluno no bloco ' . $instance->title . ' devem ser ' . $operators_array[$restriction->properator] . ' ' . $restriction->prpoints . ($restriction->properator == BETWEEN ? (' e ' . $restriction->prpointsbetween) : '') . ' pontos' . '</td><td>' . html_writer::link($url, 'Remover') . '</td></tr>';
			}
